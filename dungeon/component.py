...
class ColliderComponent:
    """엔티티의 충돌 경계 및 속성을 정의합니다."""
    width: int
    height: int
    offset_x: int = 0
    offset_y: int = 0
    is_solid: bool = True # 다른 엔티티가 통과할 수 없는지 여부
...
@dataclass
class AIComponent:
    """몬스터의 AI 상태 및 목표를 저장합니다."""
    state: str = 'IDLE' 
    target_entity_id: int = None
    last_known_player_pos: Tuple[int, int] = None
    action_cooldown: float = 0.0 # 몬스터의 다음 행동까지 남은 시간
    action_delay: float = 1.0 # 몬스터가 한 번 행동하고 나서 다음 행동까지 걸리는 시간 (초)
