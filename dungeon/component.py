...
class ProjectileComponent:
    ...
    dx: int # 이동 방향 x
    dy: int # 이동 방향 y
    impact_effect: dict = field(default_factory=dict) # 충돌 시 이펙트 정보
...
class InteractableComponent:
    ...
    data: dict = field(default_factory=dict) # 상호작용에 필요한 추가 데이터
...
class InventoryComponent:
    ...
    items: dict = field(default_factory=dict) # {item_id: {'item': Item, 'qty': int}}
    equipped: dict = field(default_factory=dict) # {slot: Item}
...
class EquipmentComponent:
    ...
    equipped_items: dict = field(default_factory=dict) # {slot: item_id}
