...
- **ECS 아키텍처 완전 전환**: 모든 게임 로직(이동, 전투, AI)을 ECS 시스템으로 분리하여 확장성을 확보했습니다.
- **스테미나(Stamina) 시스템**: 이동 시 스테미나가 소모되며, 0이 되면 탈진하는 자원 관리 요소를 추가했습니다.
- **속성 시스템 (Elemental System)**: 물 > 불 > 나무 > 흙 > 물의 순환 상성 체계를 도입하여 전략적 전투를 강화했습니다. 상성 우위 시 25% 추가 데미지가 적용됩니다.
- **원거리 및 방향성 공격 시스템**: `Space` 키를 활용한 공격 모드를 도입하여 특정 방향으로 사거리 내 모든 적을 일직선 타격(관통)할 수 있는 시스템을 구현했습니다.
