...
### 2025년 12월 20일 토요일 (UI 고도화 및 시스템 확장)
- **맵 스크롤 및 카메라 시스템**: 맵 크기를 **120x60**으로 확장하고 플레이어 중심의 카메라 스크롤 기능을 도입하여 더 넓은 세계를 탐험할 수 있게 했습니다.
- **인벤토리 불투명화 및 개선**: 팝업형 인벤토리의 배경을 불투명하게 처리하여 가독성을 높였으며, 아이템 장착 로직(두 손 장착 등)을 심화 구현했습니다.
- **한글 광폭 문자 완벽 대응**: `Renderer`를 개선하여 한글이 터미널에서 차지하는 너비를 정확히 계산, 줄 바꿈 현상으로 인한 UI 깨짐과 잔상 문제를 근본적으로 해결했습니다.
- **레이아웃 안정화**: 맵, 사이드바, 하단 스탯 창 사이의 구분선을 복구하여 시각적 정렬을 강화했습니다.
- **무기 시스템**: 원핸드/투핸드 무기 구분 및 방패 장착 로직을 추가했습니다.
