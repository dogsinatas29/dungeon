...
class DungeonMap:
    ...
    def toggle_fog(self):
        ...
        if not self.fog_enabled:
            # 안개를 끌 때, 모든 타일을 visited에 추가
            self.visited.update([(x, y) for y in range(self.height) for x in range(self.width)])
        else:
            # 안개를 다시 켤 때, 현재 시야만 남김
            self.visited = set()
            self.reveal_tiles(self.player_x, self.player_y)
    ...
    def get_tile_for_display(self, x, y, player_x, player_y):
        ...
        for m in self.monsters:
            if not m.dead and (x, y) == (m.x, m.y):
                return m.char
        if (x, y) == (self.start_x, self.start_y):
            return START
        if (x, y) == (self.exit_x, self.exit_y):
            return self.exit_type
        if (x, y) in self.room_entrances:
            # 방 번호를 타일에 포함하지 않고, 순수 'R'만 반환
    ...
    def from_dict(cls, data):
        ...
        d_map = cls(level, ui_instance=None)
