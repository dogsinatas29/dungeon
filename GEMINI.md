...
### 7. 고정 밸런스 데이터 (Balance Constants) - 2026-01-10 확정
*   **시뮬레이션 기반 확정 데이터**:
    *   이 데이터는 `dungeon/config.py`에 상수로 고정되어 있으며, 특별한 이유 없이 변경하지 않습니다.
    *   **강화 성공률 (Success Rates)**:
        *   +0 → +1: 90%
        *   +1 → +2: 80%
        *   +2 → +3: 70%
        *   +3 → +4: 60%
    ...
    *   **드롭 확률 (Drop Rates)**:
        ...
        *   51층+: 15~25%
    *   **골드 획득 (Gold Drop)**:
        *   몬스터 처치 시 **100% 확률**로 골드 드롭.
        *   공식: `Base(10) + Floor * Scaling(5)` (대략 층당 5골드씩 증가)